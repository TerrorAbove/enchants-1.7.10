...
return handleClientMessage(ExtraEnchantsMain.proxy.getPlayerEntity(ctx), message, ctx);
} else {
// server side proxy will return the server side EntityPlayer
return handleServerMessage(ExtraEnchantsMain.proxy.getPlayerEntity(ctx), message, ctx);
}
}
}