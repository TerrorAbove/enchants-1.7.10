		...
		if(event.entityLiving != null)
		{
			...
			if(list != null)
			{
				if(list.size() > 0 && !event.entityLiving.isDead)
				{
					...
					{
						PacketDispatcher.sendToAllAround(new UpdateBleedIconMessage(list.toArray(new Bleed[]{})), event.entityLiving.worldObj.provider.dimensionId,
								event.entityLiving.posX, event.entityLiving.posY, event.entityLiving.posZ, 50.0);
					}
				}
				else
				{
