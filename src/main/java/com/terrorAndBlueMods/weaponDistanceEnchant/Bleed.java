...
public class Bleed
{
	...
	public void toBytes(ByteBuf buf)
	{
		...
		buf.writeFloat(damageRemaining);
		buf.writeFloat(damageFactor);
		buf.writeInt(elapsed);
		buf.writeInt(timeLeft);
		buf.writeInt(frequency);
	}
	
	public static Bleed fromBytes(ByteBuf buf)
	{
		Bleed b = new Bleed();
		
		b.damageDealt = buf.readFloat();
		b.damageRemaining = buf.readFloat();
		b.damageFactor = buf.readFloat();
		b.elapsed = buf.readInt();
		b.timeLeft = buf.readInt();
		b.frequency = buf.readInt();
		
		//this will only be used client-side, so we don't need to validate the numbers
